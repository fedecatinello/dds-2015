...
public class Sistema implements RepositorioRecetas {
	...
	private boolean encontre(Receta receta) {
		...
		for (int i = 0; i < this.recetaConocidas.size(); i++) {
			if ((this.recetaConocidas.get(i).getNombre().equals(receta
					.getNombre()))) {
				return true;
			}
		}
	...
	/*
	 * private void purificarLista() { List<Receta> copia =
	 * this.recetaConocidas; for (int j = 0; j < copia.size(); j++) { Boolean
	 * flag = false; for (int i = 0; i < this.recetaConocidas.size(); i++) {
	 * if(i!=j){ if
	 * ((this.recetaConocidas.get(i).getNombre().equals(copia.get(j)
	 * .getNombre()))) {
	 * 
	 * flag = true;
	 * 
		...
		if (usuario.getGruposAlQuePertenece().isEmpty()
			...
			for (GrupoDeUsuarios grupo : usuario.getGruposAlQuePertenece()) {
				for (Usuario miembroDelGrupo : grupo.getUsuarios()) {
					for (Receta recetasDelMiembro : miembroDelGrupo
							.getRecetas()) {

						Boolean flag = false;
						for (int i = 0; i < recetasQueConoce.size(); i++) {
							if ((recetasQueConoce.get(i).getNombre()
									.equals(recetasDelMiembro.getNombre()))) {
								flag = true;
							}
						}
						if (flag == false) {
							recetasQueConocePorLosMiembrosDelGrupo
									.add(recetasDelMiembro);
						}
